...
import { GLPIApiAuthenticationType, GLPIAuthenticationResponse } from "../interfaces/services/authentication.model";
...
export class GLPIApiAuthenticationService{
    ...
    public async withUserToken(token: GLPIApiAuthenticationType['user_token']){
        return this.webRequest.create(this.glpiApi.glpi_url, {
            headers: {
                "App-Token": this.glpiApi.app_token,
                "Authorization": `user_token ${token}`
            }
        }).get<GLPIAuthenticationResponse>('initSession').then(response => {
